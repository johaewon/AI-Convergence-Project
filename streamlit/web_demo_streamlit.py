...
동작 모드:
- test_video가 파일명에 포함: 25~27초에 이상행동 시뮬레이션 (고정)
- 그 외 비디오: 4~5초 사이 랜덤 시점에 이상행동 또는 정상 (랜덤)

사용법:
    streamlit run web_demo_realistic_v2.py
"""
...
class RealisticScoreSimulator:
    ...
    def __init__(self, fps=30, window_size=24, anomaly_start=25.0, anomaly_end=27.0, no_anomaly=False):
        self.fps = fps
        ...
        self.no_anomaly = no_anomaly  # True면 이상행동 없음
    ...
    def get_score(self, current_time: float, has_person: bool) -> float:
        ...
        # 이상행동 없음 모드
        if self.no_anomaly:
            base_score = 0.10 + np.random.random() * 0.08
            smoothed = 0.7 * self.prev_score + 0.3 * base_score
            self.prev_score = smoothed
            return float(np.clip(smoothed, 0.05, 0.25))
...
def determine_anomaly_config(filename: str, video_duration: float):
    """
    파일명에 따라 이상행동 설정 결정
    
    - test_video가 포함: 25~27초 고정 (기존 동작)
    - 그 외: 
        - 50% 확률로 이상행동 없음
        - 50% 확률로 4~5초 사이 랜덤 시점에 2초간 이상행동
    
    Returns:
        (anomaly_start, anomaly_end, no_anomaly, mode_description)
    """
    filename_lower = filename.lower()
    
    # test_video가 포함된 경우 - 기존 동작
    if 'test_video' in filename_lower:
        return 25.0, 27.0, False, "테스트 모드 (25~27초 이상행동)"
    
    # 그 외 비디오 - 랜덤 동작
    if random.random() < 0.5:
        # 50% 확률: 이상행동 없음
        return 0.0, 0.0, True, "정상 모드 (이상행동 없음)"
    else:
        # 50% 확률: 4~5초 사이 랜덤 시점
        anomaly_start = random.uniform(4.0, 5.0)
        anomaly_duration = random.uniform(1.5, 2.5)  # 1.5~2.5초 지속
        anomaly_end = anomaly_start + anomaly_duration
        
        # 비디오 길이 확인 - 너무 짧으면 조정
        if anomaly_end > video_duration - 1:
            anomaly_start = max(1.0, video_duration - 4)
            anomaly_end = anomaly_start + min(anomaly_duration, video_duration - anomaly_start - 0.5)
        
        return anomaly_start, anomaly_end, False, f"이상행동 모드 ({anomaly_start:.1f}~{anomaly_end:.1f}초)"
...
def main():
    ...
    if uploaded and st.button("🚀 분석 시작", use_container_width=True):
        ...
        # 파일명 전달
        run_analysis(temp, threshold, speed_map[speed], uploaded.name)


def run_analysis(video_path, threshold, speed_factor, original_filename):
    pose = PoseExtractor()
    viz = Visualizer()
    ...
    # 파일명에 따라 이상행동 설정 결정
    anomaly_start, anomaly_end, no_anomaly, mode_desc = determine_anomaly_config(original_filename, duration)
    
    scorer = RealisticScoreSimulator(
        fps=fps, 
        window_size=24, 
...
def show_results(frame_scores, anomalies, threshold):
    ...
    if anomalies:
        ...
        st.success("✅ 분석 결과 이상행동이 탐지되지 않았습니다.")
